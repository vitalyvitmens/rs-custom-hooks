...
### Действия по выполнению кода с пользовательским хуком useToggle и компонентом AdditionalTask2:

####  Для создания хука useToggle использовал хук useReducer, который позволяет управлять сложным или зависимым состоянием.

####  Определил начальное состояние, которое содержит индекс текущего значения и массив значений, которые передал в хук useToggle. Если не передавать никаких значений, то по умолчанию используется массив [true, false].

####  Написали функцию reducer, которая обрабатывает два типа действий: "toggle" и "set". Действие "toggle" переключает индекс на следующее значение в массиве, а действие "set" устанавливает индекс в соответствии с переданным значением, если оно есть в массиве.

####  Использовал хук useReducer внутри моего кастомного хука useToggle и вернул текущее значение и функцию dispatch, которую назвал toggle.

####  Для использования хука useToggle создал два компонента Button, которые принимают функцию onClick и дочерний элемент. Также импортировали стили из файла tasks.module.css.

####  Вызвал хук useToggle два раза: один раз с массивом ['blue', 'orange', 'cyan', 'teal'], чтобы получить значение и функцию toggle для цвета divа кнопки и текста кнопки, и один раз с массивом ['light', 'dark'], чтобы получить значение и функцию toggleTheme для темы компонента AdditionalTask2.

####  Определил объект styleTheme, который зависит от значения theme и задает цвет фона и текста для темы компонента AdditionalTask2.

####  Вернул JSX-разметку, которая содержит два компонента Button: один для изменения цвета, а другой для изменения темы. Также добавил некоторые стили для выравнивания и оформления элементов.
